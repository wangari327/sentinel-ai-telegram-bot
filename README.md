...
If `TVWEB_DATABASE_URL` is configured, the bot can steer users to ibox-tv.com results. Support-looking messages search the local catalog cache first, then use the website `tv_shows` table only when the cache misses. If no match is found and the request passes the logging guard, it records the request so you can review demand later from `/panel`. Use a read-only database user for this connection where possible.

If `TMDB_BEARER_TOKEN` is configured, Sentinel checks release metadata before logging release-date and missing-episode style messages. Future movies, unaired seasons, unaired episodes, and unconfirmed seasons get a direct availability reply instead of noisy dashboard tickets. User-facing replies do not expose the metadata source; they just answer the question. This is especially useful for messages like "Silo season 3 episode 8 missing" when the episode has not aired yet.

When a user asks for a specific season or episode but Sentinel finds the base title on ibox-tv.com, it does not blindly log the exact part as missing. It replies with the current/search result and asks the user to search first, because some older seasons may exist in the Telegram channel/file store even if the website row only shows the newest post. It only writes dashboard work when the message is a real broken/expired/banned/playback report, or when a released/aired newer part is clearly absent.
