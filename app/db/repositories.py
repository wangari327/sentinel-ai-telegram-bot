...
def get_or_create_group(
    ...
    is_moderatable = is_moderatable_chat_type(chat_type)
    ...
    if group is None:
        ...
        if not is_moderatable_chat_type(group.type):
...
def list_groups(session: Session, *, include_private: bool = False) -> list[Group]:
    ...
    if not include_private:
        query = query.where(Group.type.in_(tuple(MODERATABLE_CHAT_TYPES)))
    return list(session.scalars(query).all())
...
def set_group_authorized_by_id(session: Session, group_id: int, authorized: bool) -> Group | None:
    ...
    if group is None:
        return None
    if authorized and not is_moderatable_chat_type(group.type):
